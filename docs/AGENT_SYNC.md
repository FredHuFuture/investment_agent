...
### [2026-03-10] Task 019 Report by Claude
- **Implemented**:
  - `engine/sector.py` -- Sector rotation matrix with `SECTOR_ROTATION_MATRIX` dict (RISK_ON, RISK_OFF, NEUTRAL regimes x 11 sectors) and `get_sector_modifier(sector, regime)` function returning int modifier [-30, +30]. Returns 0 for None/unknown sector or NEUTRAL/unknown regime.
  - `engine/correlation.py` -- Portfolio correlation tracker with `calculate_portfolio_correlations(tickers, provider, lookback_days)` async function. Fetches price history in parallel via `asyncio.gather`, computes daily returns, then pairwise Pearson correlation via `pandas.DataFrame.corr()`. Returns correlation matrix, avg correlation, high-correlation pairs (>0.70), concentration risk level (HIGH/MODERATE/LOW), and warnings. Handles gracefully: <2 tickers, missing data, insufficient overlap.
  - `engine/pipeline.py` -- After aggregation (step 6), added step 7: apply sector rotation modifier for stocks only (not crypto). Reads sector from `ticker_info`, regime from `signal.regime`. Stores `sector_modifier`, `sector_name`, `pre_sector_confidence` in `signal.metrics` for report display. Confidence clamped to [30, 90].
  - `cli/report.py` -- Added sector adjustment display in two modes: (1) Standard mode: single line `Sector Adj: +20 (Technology in RISK_ON)` when modifier is non-zero. (2) Detail mode: full `SECTOR ADJUSTMENT` block with sector name, regime, modifier with flavor text, and before/after confidence. Added `_format_sector_detail()` helper.
  - `engine/__init__.py` -- Exported `SECTOR_ROTATION_MATRIX`, `get_sector_modifier`, `calculate_portfolio_correlations`.
  - `tests/test_019_sector_correlation.py` -- 15 tests covering: sector modifier for RISK_ON (+20 Tech), RISK_OFF (-15 Tech), NEUTRAL (0), unknown sector (0), unknown regime (0), confidence clamping [30, 90], matrix completeness (RISK_ON/RISK_OFF sectors match), correlation calculation (linear prices -> high corr), HIGH concentration risk, LOW concentration risk (random walks), missing data graceful handling, single ticker edge case, standard report display, detail report display, and no-display when modifier absent.
- **Skipped/Deferred**: The task spec mentioned adding correlation warnings to the WARNINGS section of the report. Since correlation is calculated on-demand (not per-analysis) and requires a separate call with multiple tickers + provider, the pipeline does not automatically compute correlations during single-ticker analysis. The `calculate_portfolio_correlations` function is available for use by a future CLI subcommand or the monitoring daemon. The report will display correlation warnings if they are manually added to `signal.warnings`.
- **Technical Concerns / Edge Cases**:
  - The correlation function uses `asyncio.gather` for parallel price fetching. On Windows with yfinance, the `_yfinance_lock` in `YFinanceProvider` serializes all yf.download calls anyway, so parallelism is limited for that provider. For test purposes, the `_FakeProvider` stub does not have this limitation.
  - The sector modifier only applies when the modifier is non-zero. If a sector is in the matrix with a 0 value (e.g., Healthcare in RISK_ON), no `sector_modifier` key is stored in metrics and no display section is shown. This is intentional -- no adjustment means nothing to report.
  - Correlation uses absolute values of correlations for avg_correlation. A highly negative correlation (e.g., -0.85) would also be flagged in `high_correlation_pairs`, which is useful since perfect inverse correlation is also a form of concentration (just inverse).
- **Questions for FutureClaw**:
